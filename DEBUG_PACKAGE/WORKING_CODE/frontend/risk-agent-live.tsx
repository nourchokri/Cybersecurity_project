...
export function RiskAgentLiveOutput({ agentSlug, onLog, onAnalysisComplete }: RiskAgentLiveOutputProps) {
      ...
      <Card className="shrink-0 rounded border-border">
        ...
        <CardContent className="space-y-3 p-3 pt-0">
            ...
            <div className="space-y-1.5">
              ...
              >
                {sampleEvents.map((evt, i) => (
                  <option key={evt.event_id} value={i}>
                    {evt.event_id} — {evt.user_id} (score: {evt.score})
                  </option>
                ))}
              </select>
            </div>
          ...
          <Button
            ...
            className="w-full gap-2 rounded-md bg-blue-600 text-sm font-medium text-white hover:bg-blue-700"
          >
            {loading ? (
              <>
