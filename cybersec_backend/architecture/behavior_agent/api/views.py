...
class ScoreSessionView(APIView):
    ...
    def post(self, request):
        ...
        # ── SPECIAL CASE: network events ─────────────────────────────
        if request.data.get("event_type") == "network_connection":
            from ..scoring.network_handler import handle_network_event

            result = handle_network_event(request.data)

            if result.get("flagged"):
                _forward_to_risk_agent(result, priority="HIGH")

            return Response(result, status=status.HTTP_200_OK)

        # ── NORMAL FLOW: behavior scoring ────────────────────────────
        ...
        thread_id = request.data.get(
            'thread_id',
            f"api_{serializer.validated_data['user_id']}"
        )
...
class NetworkBatchView(APIView):
    """
    POST /api/v1/network/analyze/batch/
    Called by Team 1's NetworkAgentClient.send_network_sessions()
        ...
        "sessions": [
            {
                "user_id": "john.doe",
                "device_id": "LAPTOP-01",
                "events": [ ...network_connection events... ]
            }
        ]
    }
    """
    ...
    def post(self, request):
        from ..scoring.network_handler import handle_network_window
        ...
        for session in sessions:
            ...
            # Inject user_id into each event so handler can find it
            for e in events:
                if not e.get("user_id"):
                    e["user_id"] = user_id

            result = handle_network_window(events)
        ...
        logger.info(
            f"NetworkBatchView: {len(results)} sessions processed, "
            f"{flagged_count} flagged"
        )

        return Response(
            {"results": results, "count": len(results), "flagged": flagged_count},
            status=status.HTTP_200_OK
        )


class NetworkHealthView(APIView):
